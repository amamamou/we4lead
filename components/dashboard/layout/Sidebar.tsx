              ...
              return (
                <li key={it.key} className="w-full">
                  <button
                    ...
                    title={it.label}
                    className={`group relative w-full flex items-center justify-center p-6 rounded-md text-gray-600 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-800 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-[#020E68]/40`}
                    aria-label={it.label}
                    aria-current={isActive ? 'true' : undefined}
                  >
                    {isActive && <span className="absolute left-0 h-10 w-1 bg-[#020E68] rounded-r" />}

                    {Icon ? <Icon size={28} /> : <span className="w-6 h-6 bg-gray-200 rounded" />}
          ...
          {(() => {
            ...
            return (
              <button
                ...
                title="Account"
                className={`group relative flex items-center justify-center w-full p-3 rounded-md text-gray-600 dark:text-gray-300 transition ${isActive ? '' : ''}`}
                aria-label="Account"
              >
              ...
              return (
                <li key={it.key} className={`${isCondensed ? 'w-auto flex-initial' : 'flex-1'} text-center`}>
                  <button
                    ...
                    onClick={() => onChange?.(it.key)}
                    title={it.label}
                    className={`flex flex-col items-center justify-center ${isCondensed ? 'w-12 px-1 py-1' : 'w-full px-2 py-2'} rounded-lg transition ${
